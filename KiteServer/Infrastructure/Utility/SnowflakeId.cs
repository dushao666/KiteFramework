...
public class SnowflakeId
{
    ...
    public static SnowflakeId Default()
    {
        lock (SLock)
        {
            ...
            if (!int.TryParse(Environment.GetEnvironmentVariable("CAP_WORKERID", EnvironmentVariableTarget.Machine), out var result))
            {
                result = random.Next(31);
            }

            if (!int.TryParse(Environment.GetEnvironmentVariable("CAP_DATACENTERID", EnvironmentVariableTarget.Machine), out var result2))
            {
