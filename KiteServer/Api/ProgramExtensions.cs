...
public static class ProgramExtensions
{
    ...
    /// 输出启动日志
    /// </summary>
    public static void LogStartupInformation(this WebApplicationBuilder builder)
    {
        ...
        Log.Information("""
                            ...
                            _  __ _  _          _____                                            _
                           | |/ /(_)| |        |  __|                                           | |
                           | ' /  _ | |_  ___  | |_  _ __  __ _  _ __ ___    ___  __      __ ___| | __
                           |  <  | || __|/ _ \ |  _|| '__|/ _` || '_ ` _ \  / _ \ \ \ /\ / // _ \ |/ /
                           | . \ | || |_|  __/ | |  | |  | (_| || | | | | ||  __/  \ V  V /|  __/   <
    ...
    public static void AddCustomDatabase(this WebApplicationBuilder builder)
    {
        ...
        // 注册自动建表服务
        builder.Services.AddSingleton<AutoGeneratedService>(sp =>
            new AutoGeneratedService(sqlSugar, builder.Configuration));

        // 注册种子数据相关服务
        // 自动注册所有 ISeedData 实现
        var seedDataTypes = typeof(ISeedData).Assembly
            .GetTypes()
            .Where(t => !t.IsInterface && !t.IsAbstract && typeof(ISeedData).IsAssignableFrom(t))
            .ToList();

        foreach (var type in seedDataTypes)
        {
            builder.Services.AddScoped(type);
            builder.Services.AddScoped(typeof(ISeedData), sp => sp.GetRequiredService(type));
        }

        builder.Services.AddScoped<SeedDataService>(sp =>
            new SeedDataService(sp.GetServices<ISeedData>(), builder.Configuration));
    }
    ...
    public static void UseCustomMiddleware(this WebApplication app)
    {
        ...
        // 执行自动建表
        var autoGeneratedService = app.Services.GetRequiredService<AutoGeneratedService>();
